...
export function migrateAiSafetyData({ topicsDoc, depsDoc, coveredResults = [], historicalBackfillStart, migratedAt }) {
...
async function main() {
  ...
  if (!dryRun && (depsDoc.generationBatches || []).some(batch => batch.id === HISTORICAL_BATCH_ID)) {
    throw new Error('AI safety migration 已应用；不得重复执行');
  }
  const { readdirSync } = await import('node:fs');
  const workDir = resolve(DATA, '.granularity-work');
  const coveredResults = readdirSync(workDir).filter(file => file.endsWith('.json')).sort()
  ...
  const out = migrateAiSafetyData({
    topicsDoc, depsDoc, coveredResults,
    historicalBackfillStart: Math.max(0, depsDoc.dependencies.length - 329),
