...
- [x] **AI 安全闸门**（260 个复用 ID 家族的 761 条继承边全部隔离重审，301 条旧 reviewed 降级为 machine；49 个 covered 父主题退出儿童视图；儿童路径只使用 reviewed 且非 rescope 的边）
- [ ] **人工校对**（`machine` → `reviewed/rejected`，优先处理 761 条 `rescopeRequired`；当前覆盖率 37.6%）
