...
> **状态：** `v1.2.0-zh.0` · 已翻译微主题 1,590 / 1,590 · 中国特有微主题 2,008 · 上游依赖 3,221 ·
> 中国特有依赖 2,619 · 内部边审核 reviewed 1,326 / machine 873 / rejected 420（50.6%）
>
> **成熟度：alpha（数据集，尚未经教师审核）。** 全部中文文本为机器翻译——1,586 / 1,590 条
> 微主题标记 `translationStatus: machine`，仅 4 条经人工校对；中国特有依赖中 873 条
> `reviewStatus: machine`，是未审核的 AI 推测边，**不进入儿童路径**。**AI 复审
> （`reviewProvenance: ai-consensus`）不等同于教师人工审核。**`cnStandards` /
...
| **教材来源元数据** | 1,069 个教材来源节点，含 290 个具体阅读文本节点 | 作品名、部分作者名和来源作为事实性元数据保留；项目不主张独占版权 |
